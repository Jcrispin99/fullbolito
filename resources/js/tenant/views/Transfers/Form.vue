...
watch(
    ...
    (newData) => {
        if (newData && Object.keys(newData).length > 0) {
            form.value = {
                from_warehouse_id: newData.from_warehouse_id || undefined,
                to_warehouse_id: newData.to_warehouse_id || undefined,
...
const warehouseTarget = ref<"from" | "to">("from");
...
// Exclude the warehouse chosen on the opposite side to prevent same-warehouse.
const fromWarehouseOptions = computed(() =>
    warehouseOptions.value.filter(
        (o: any) => o.value !== form.value.to_warehouse_id,
    ),
);
const toWarehouseOptions = computed(() =>
    warehouseOptions.value.filter(
        (o: any) => o.value !== form.value.from_warehouse_id,
    ),
);

function openWarehouseCreate(target: "from" | "to", name: string) {
    warehouseTarget.value = target;
    warehouseDialog.open(name);
}

function openWarehouseEdit(target: "from" | "to", id: number) {
    warehouseTarget.value = target;
...
async function onWarehouseSubmit(payload: any) {
    ...
    if (record) {
        ...
        if (warehouseTarget.value === "from") {
            form.value.from_warehouse_id = record.id;
        } else {
            form.value.to_warehouse_id = record.id;
        }
    }
}
...
const handleGhostProductSelect = (
    ...
    ghostData.value = { product_product_id: undefined, quantity: 1 };
...
<template>
    <div class="space-y-2">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <SearchSelect
                                ...
                                placeholder="Buscar almacén origen..."
                                :show-create="isDraft"
                                :show-edit="true"
                                @create="openWarehouseCreate('from', $event)"
                                @edit="(id) => openWarehouseEdit('from', id)"
                            />
                            <p
                                v-if="errors?.from_warehouse_id"
                        ...
                        <div class="space-y-2">
                            ...
                            <SearchSelect
                                ...
                                placeholder="Buscar almacén destino..."
                                :show-create="isDraft"
                                :show-edit="true"
                                @create="openWarehouseCreate('to', $event)"
                                @edit="(id) => openWarehouseEdit('to', id)"
                            />
                            <p
                                v-if="errors?.to_warehouse_id"
                                class="text-sm text-destructive"
                            >
                    ...
                    <div class="overflow-x-auto min-h-[260px] px-2">
                        <table class="w-full border-collapse">
                            <thead>
                                ...
                                >
                                    <th
                                        class="pt-2 pb-3 px-2 font-medium min-w-[300px] w-[55%]"
                                    >
                                        Producto
                                    </th>
                                    <th
                                                ...
                                                <Button
                                                    ...
                                                    :title="
                                                        form.from_warehouse_id
                                                            ? 'Seleccionar lote del almacén origen'
                                                            : 'Selecciona primero el almacén origen'
                                                    "
                                                    :disabled="
                                                        !form.from_warehouse_id
                                                    "
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <p class="text-xs text-muted-foreground">
                            Compañía que emite la transferencia (para la
                            numeración).
                        </p>
                    </div>
                </div>
        ...
        <TransferLotModal
            ...
            :warehouse-id="form.from_warehouse_id ?? null"
