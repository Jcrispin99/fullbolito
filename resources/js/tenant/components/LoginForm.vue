  ...
  <form @submit.prevent="handleLogin" :class="cn('flex flex-col gap-6', props.class)">
    <FieldGroup class="gap-5">
      <div class="text-left">
        ...
        <h1 class="text-3xl font-bold tracking-tight sm:text-4xl">Vuelve al juego.</h1>
        <p class="mt-2 text-sm leading-6 text-muted-foreground">
          Ingresa con tus credenciales para administrar tu negocio.
        </p>
      </div>

      <div v-if="error" class="rounded-xl border border-destructive/25 bg-destructive/10 p-3.5 text-sm text-destructive" role="alert">
        {{ error }}
      </div>
      ...
      <div class="rounded-xl border border-dashed border-border bg-muted/35 px-4 py-3 text-center text-sm text-muted-foreground">
        ...
        <RouterLink :to="{ name: 'Home' }" class="ml-1 font-semibold text-primary underline-offset-4 hover:underline">
          Volver al sitio
        </RouterLink>
      </div>
